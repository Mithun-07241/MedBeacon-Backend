...
exports.completeProfile = async (req, res) => {
    try {
        const { User, PatientDetail, DoctorDetail } = req.models;
        ...
        let saved;
        if (role === 'patient') {
            saved = await PatientDetail.findOneAndUpdate({ userId }, { ...profileData, userId }, { upsert: true, new: true });
        } else if (role === 'doctor') {
            saved = await DoctorDetail.findOneAndUpdate({ userId }, { ...profileData, userId }, { upsert: true, new: true });
        } else {
        ...
        res.status(201).json({ message: 'Profile saved successfully', details: saved });
    } catch (error) {
...
exports.getProfileDetails = async (req, res) => {
    try {
        ...
        if (role === 'patient') {
            ...
            return res.json({ patient: { ...patientData, username: user?.username, email: user?.email } });
        } else if (role === 'doctor') {
            const details = await DoctorDetail.findOne({ userId });
            const user = await User.findOne({ id: userId });
            const doctorData = details ? details.toObject() : {};
            return res.json({ doctor: { ...doctorData, username: user?.username, email: user?.email } });
        } else {
            return res.status(400).json({ error: 'Invalid role' });
        }
    } catch (error) {
...
exports.getPatientById = async (req, res) => {
    try {
        const { User, PatientDetail } = req.models;
        const { id } = req.params;
        const role = req.user.role;

        if (role === 'patient' && id !== req.user.id) return res.status(403).json({ error: 'Access denied' });
...
exports.updateCompletionStatus = async (req, res) => {
    try {
        ...
        if (typeof profileCompleted !== 'boolean') return res.status(400).json({ error: 'Must be boolean' });

        const updated = await User.findOneAndUpdate({ id: req.user.id }, { profileCompleted }, { new: true });
...
exports.getPatients = async (req, res) => {
    try {
        const { User } = req.models;
        if (!['doctor', 'admin', 'clinic_admin'].includes(req.user.role)) return res.status(403).json({ error: 'Access denied' });
        ...
        const patients = await User.aggregate([
            ...
            {
                $project: {
                    _id: 0, id: 1, username: 1, email: 1, role: 1, profilePicUrl: 1,
                    firstName: '$details.firstName', lastName: '$details.lastName',
                    phoneNumber: '$details.phoneNumber', age: '$details.age',
                    gender: '$details.gender', dateOfBirth: '$details.dateOfBirth',
                    address: '$details.address', allergies: '$details.allergies',
                    treatmentFileUrl: '$details.treatmentFileUrl', bio: '$details.bio',
                    emergencyContactName: '$details.emergencyContactName',
                    emergencyContactPhone: '$details.emergencyContactPhone',
                    bloodType: '$details.bloodType', medicalHistory: '$details.medicalHistory'
                }
            }
        ]);
        ...
        const doctors = await User.aggregate([
            ...
            { $unwind: { path: '$details', preserveNullAndEmptyArrays: true } },
            {
                $project: {
                    ...
                    firstName: '$details.firstName', lastName: '$details.lastName',
                    phoneNumber: '$details.phoneNumber', age: '$details.age',
                    gender: '$details.gender', experience: '$details.experience',
                    dateOfBirth: '$details.dateOfBirth', address: '$details.address',
                    specialization: '$details.specialization', proofFileUrl: '$details.proofFileUrl',
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        if (Object.keys(userUpdates).length > 0) {
            await User.findOneAndUpdate({ id: userId }, userUpdates);
        }

        if (!updated) return res.status(404).json({ error: 'Profile not found' });

        res.json({ message: 'Profile updated successfully', [role]: updated });
    } catch (error) {
...
exports.uploadTreatmentFile = async (req, res) => {
    try {
        const { PatientDetail } = req.models;
        const userId = req.user.id;
        const role = req.user.role;

        if (role !== 'patient') return res.status(403).json({ error: 'Only patients can upload treatment files' });
        if (!req.file) return res.status(400).json({ error: 'No file uploaded' });

        const treatmentFileUrl = req.file.path;
        const updated = await PatientDetail.findOneAndUpdate({ userId }, { treatmentFileUrl }, { new: true });

        if (!updated) return res.status(404).json({ error: 'Patient profile not found' });

        res.json({ message: 'Treatment file uploaded successfully', treatmentFileUrl });
    } catch (error) {
