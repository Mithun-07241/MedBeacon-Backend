...
exports.getInventoryItems = async (req, res) => {
    try {
        const { InventoryItem, User } = req.models;
        ...
        const items = await InventoryItem.find(match).sort({ createdAt: -1 }).limit(parseInt(limit)).lean();

        const assignedUserIds = items.filter(i => i.assignedTo).map(i => i.assignedTo);
        let userMap = {};
        if (assignedUserIds.length > 0) {
            const users = await User.find({ id: { $in: assignedUserIds } }).select('id username email role');
            users.forEach(u => { userMap[u.id] = u; });
        }

        const enrichedItems = items.map(item => ({ ...item, assignedUser: item.assignedTo ? userMap[item.assignedTo] || null : null }));
...
exports.getInventoryItemById = async (req, res) => {
    try {
        ...
        if (item.assignedTo) {
            item.assignedUser = await User.findOne({ id: item.assignedTo }).select('id username email role');
        }
...
exports.assignItem = async (req, res) => {
    try {
        ...
        await item.save();
        res.json({ message: userId ? 'Item assigned successfully' : 'Item unassigned successfully', item });
    } catch (error) {
...
exports.getItemsByCategory = async (req, res) => {
    try {
        ...
        const items = await InventoryItem.aggregate([
            { $group: { _id: '$category', count: { $sum: 1 }, totalValue: { $sum: { $multiply: ['$quantity', '$purchasePrice'] } }, items: { $push: '$$ROOT' } } },
            { $sort: { count: -1 } }
        ]);
    ...
    try {
        ...
        const csv = [
            ...
            ...items.map(item => [
                ...
                `"${item.location}"`, new Date(item.purchaseDate).toISOString().split('T')[0],
                item.purchasePrice, `"${item.supplier}"`, item.status, item.assignedTo || 'N/A'
            ].join(','))
        ].join('\n');
